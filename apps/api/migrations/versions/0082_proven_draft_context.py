"""Admit source-proven context and range reconciliation before fresh verification."""

from collections.abc import Sequence

from alembic import op

revision: str = "0082_proven_draft_context"
down_revision: str | Sequence[str] | None = "0081_explicit_unit_draft"
...
def _admission(expanded: bool) -> None:
    ...
    current = previous[:-1] + ",'retained-policy-association-v13')"
    if not expanded:
        ...
        op.execute(f"""
          DO $$ DECLARE definition TEXT; BEGIN
            ...
            IF strpos(definition,$previous${previous}$previous$)=0 THEN
              RAISE EXCEPTION 'proven draft context contract changed';
            END IF;
    ...
    revisions = (
        ...
        "'policy-draft-normalization-v5','policy-draft-normalization-v6'"
    )
    if expanded:
        revisions += ",'policy-draft-normalization-v7'"
    op.execute(f"""
    ...
    op.execute("""
      DO $$ BEGIN
        IF EXISTS(SELECT 1 FROM policy_structuring_jobs
            WHERE pipeline_version='retained-policy-association-v13')
          OR EXISTS(SELECT 1 FROM policy_range_replay_sources
            WHERE normalization_revision='policy-draft-normalization-v7') THEN
          RAISE EXCEPTION 'proven draft context history prevents downgrade' USING ERRCODE='23514';
        END IF;
      END $$;
    """)
    _admission(False)
