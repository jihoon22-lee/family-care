"""Preserve metadata history while distinguishing reading-guide legends."""

from collections.abc import Sequence

from alembic import op

revision: str = "0051_metadata_reading_guides"
down_revision: str | Sequence[str] | None = "0050_metadata_instructions"
        ...
        ALTER TABLE document_metadata_publications
          ...
          ADD CONSTRAINT document_metadata_publications_validator_revision_check CHECK(
            validator_revision IN ('document-metadata-api-v1','document-metadata-api-v2',
              'document-metadata-api-v3','document-metadata-api-v4','document-metadata-api-v5','document-metadata-api-v6','document-metadata-api-v7'));
    """)
...
def downgrade() -> None:
    op.execute("""
        DO $$ BEGIN
          IF EXISTS(SELECT 1 FROM document_metadata_proposals WHERE revision='document-metadata-v7')
            OR EXISTS(SELECT 1 FROM document_metadata_publications
              WHERE validator_revision='document-metadata-api-v7') THEN
