"""Append metadata proofs for a geometrically proven prefix without losing history."""

from collections.abc import Sequence

from alembic import op

revision: str = "0064_metadata_proven_prefix"
down_revision: str | Sequence[str] | None = "0063_review_claim_sources"
...
def downgrade() -> None:
    op.execute("""
        DO $$ BEGIN
          IF EXISTS(SELECT 1 FROM document_metadata_proposals WHERE revision='document-metadata-v8')
            OR EXISTS(SELECT 1 FROM document_metadata_publications
              WHERE validator_revision='document-metadata-api-v8') THEN
        ...
        ALTER TABLE document_metadata_publications
          ...
          ADD CONSTRAINT document_metadata_publications_validator_revision_check CHECK(
            validator_revision IN ('document-metadata-api-v1','document-metadata-api-v2',
              'document-metadata-api-v3','document-metadata-api-v4','document-metadata-api-v5',
              'document-metadata-api-v6','document-metadata-api-v7'));
    """)
