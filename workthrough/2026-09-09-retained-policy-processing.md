...
CI `34341189086`의 전체 PostgreSQL은 798건 통과/2건 실패(1399.85초)였다.
`date_origin_database`의 `TRUNCATE document_structure_generations CASCADE`가 새 FK를 따라
이미 claim한 테스트 작업까지 삭제한 것이 원인이었다. `b2a25d9`는 선행 TRUNCATE를
제거하고 제품 잠금 가드와 실제 계약일/보장 시작일을 구분하는 assertions를 유지한다.
동일 실패 2건(2.91초)을 재현한 뒤 2건 통과(3.37초)를 확인했다. 관련 3개 모듈은
54건 통과/1건 환경 실패(67.90초)였다. 마지막 실패는 schema 0067 합성 DB에서 0066
