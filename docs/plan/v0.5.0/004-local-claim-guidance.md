...
5. in_progress — API/UI/snapshot과 stale 판정을 새 계약으로 연결했다. 과거 청구/실수령/계산 snapshot을 보존하고 사건·가입/연결·원문·지식·가정 revision으로 재현한다. private와 operational 청구의 중복·동시 생성·실패 롤백을 검증했고, 최종 검토에서 발견한 복원 충돌과 재import 후 과거 출처 연결을 보완한다.
