...
3. in_progress — 범위별 검토 후보와 원문 위치, 필드 원문 대조, 로컬 피보험자/계약
   연결 근거를 저장한다. 자동 원장 반영·component/판본 연결·공통 담보 identity·중복 제거·
   사용자 교정 우선순위의 실제 소비 경로는 후속이다.
4. in_progress — 문서 등록의 범위별 구조화·최소화·총량 제한을 연결한다. OpenAI 연결 작업은
   ...
   전체 IR 범위 저장/재개와 후보 publication은 합성 통합에 연결했다. 기본 실행 설정
   전환은 자동 원장 projector와 함께 연결할 후속 작업이다.
