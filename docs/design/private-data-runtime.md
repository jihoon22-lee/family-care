...
2. 사용자가 필요한 파일을 저장소 밖 import directory로 수동 다운로드한다.
...
- 성공·실패·취소·process shutdown에서 password buffer 참조를 폐기한다.
- 재분석은 관리 archive를 사용하므로 정상 import 후 원본 PDF password를 매번 요구하지 않는다.

## Archive key

- `FAMILYCARE_ARCHIVE_MASTER_KEY_FILE`은 저장소 밖 mode `0600` local file을 가리킨다.
- Compose는 key file을 Worker/API의 필요한 process에 read-only secret mount한다.
...
OpenAI document structuring은 사용자가 승인한 v0.1 기능이다. PDF binary와 page image는 전송하지 않고 Worker가 추출한 필요한 text batch만 전송한다. 세부 최소화와 검증은 `docs/design/ai-document-analysis.md`를 따른다.

## Pragmatic security baseline
...
실제 자료 검증은 사용자가 지정한 저장소 밖 path와 문서에만 수행한다.
...
7. 확인하지 않은 보험사·문서 형식·모바일 기기는 미검증으로 남긴다.
...
- 원본 Google Drive 파일은 어떤 실패 경로에서도 수정·삭제하지 않는다.
...
3. Google Drive 원본은 v0.1이 수정하거나 삭제하지 않는다.
