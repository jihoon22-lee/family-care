        ...
        for component in source["proposal_json"]["components"]:
            ...
            if valid:
                overlap = connection.execute(
                    ...
                    "AND (c.document_version_id=%s OR version.content_sha256=("
                    "SELECT content_sha256 FROM document_versions WHERE id=%s)) "
                    "AND NOT(c.page_end<%s OR c.page_start>%s) LIMIT 1",
                    (
            ...
            component_id = uuid4() if outcome == "APPLIED" else None
            connection.execute(
