...
진행 중인 개발에 대한 예산 승인·상태 질문·설명 요청은 기존 목표를 대체하지 않는다.
답변은 중간 업데이트로 제공하고 승인된 구현·반영을 이어간다. 예산이 승인됐다는 사실을
보고하는 것만으로 작업을 종료하지 않는다. 필수 입력이나 실제 한도에 막힌 경우에도 그와
독립적인 작업은 진행하고, 사용자가 전체 작업을 취소한 경우에만 원래 범위를 중단한다.

## Verification cadence

커밋마다 변경 파일의 문법·형식과 기존 계획 범위만 확인한다. PR의 계획 작업을 모두
구현한 뒤 상세 검증과 전체 diff 검토를 한 번 집중 수행한다. 테스트는 구현과 함께
작성하되 개발 중 실행은 구체적 문제를 해결하는 최소 범위로 제한한다.

동일 입력의 CI 결과를 활용해 로컬 전체 suite·통합·빌드를 중복하지 않는다. 실패 수정
후에는 영향받는 검사만 다시 실행한다. 새 SHA·문서 수정·세션 재개 때문에 검증을
처음부터 반복하지 않는다. 검증 중 발견한 후속 기능은 현재 PR 수용에 필수인지 판단해
범위를 정하며, 불필요한 진단 확장으로 완료 시점을 계속 미루지 않는다.

상세 명령과 증거 재사용 기준은 [검증 전략](design/test-strategy.md#verification-by-change)이 소유한다.
