...
### Explicit native row unit recovery

명시적 retained v12 / normalization v6 / schema 0081은 AI 초안이 이미 인용한 단일 native
가입 행에서 원래 숫자와 명시적 단위의 관계를 입증하여 누락된 배율을 적용한다. 같은 행의
정확한 가입명과 금액을 함께 입증할 때만 이름·금액 초안을 원자적으로 복구한다. 다른 행의
숫자, 추정 단위·통화, 모호한 기하 관계와 최소화에서 제거한 값을 사용하지 않는다. v1–v5의
판독 의미와 기존 raw 응답·거부 이유는 유지하며, 새 초안은 v11의 문맥 폐쇄를 유지한 검수와
프로그램 근거 검사를 다시 거친다. 가입금액은 지급액이나 지급 산식의 대체물이 아니다.

API는 같은 source generation·물리 담보·구성원·원장 버전의 이전 프로그램 게시에 금액과
통화가 모두 비어 있는 경우만 보충한다. 기존 값의 덮어쓰기와 사용자 교정·검토·삭제의
우회를 금지하며 독립 native 금액 근거와 원래 제거 이유·새 조정 receipt를 대조한다.
변경하지 않은 원장 필드와 기존 publication·청구 snapshot은 유지한다.

기존 최소화 revision의 fingerprint만 달라진 경우 v12는 현재 구성원과 최소화 revision의
