...
- 상태: complete (명시된 최종 수용·릴리스·운영 전환) / PARTIAL (자료 지원); 이슈 종료 동기화 준비
...
6. complete (버전 복구·기능 CI·태그) / complete (릴리스) / complete (운영 전환) / complete (네트워크 수용) — PR #94로 조기 v0.5.x 게시를 정리하고 개발·최종 버전을 v0.5.0으로 통일했다. PR #106/#107의 필수 CI 통과와 `v0.5.0` 태그 게시를 확인했다. 릴리스 실행 34719394060과 GitHub Release 게시가 성공했다. 운영 전환과 세 이미지/health/readiness 일치를 확인했다. WSL curl 네트워크 수용도 통과했다. 최종 문서 PR 통합 후 이슈 종료 판단을 동기화한다. 이전 게시/전환 증거는 역사이며 새 실행으로 합산하지 않는다.
7. complete (최종 수용 판단) / PENDING (GitHub 동기화) — 과거 #59/#60/#62/#63/#69/#70의 OPEN 판단과 당시 제한은 보존한다. 최종 수용·릴리스·운영 전환의 명시 범위는 완료로 판단하며 문서 PR 통합 후 #60/#63/#69/#70에 동기화한다. 전 자료 100% 자동 확정·넓은 AI 추가 평가·모바일 실기기 PASS를 임의의 종료 gate로 추가하지 않는다. 미지원/PARTIAL과 기기 미검증을 명시하고, 배포 성공을 전체 지급/판독 품질 PASS로 올리지 않는다.
...
현재 태그 소스는 `7d1a53d`/schema 0083다. PR #103/#105/#106/#107은 MERGED다.
PR #106/#107은 각각 필수 CI 7/7·PostgreSQL 944/947개를 통과했다.
[release 34719394060](https://github.com/jihoon22-lee/family-care/actions/runs/34719394060)는
SUCCESS이며 [정식 Release](https://github.com/jihoon22-lee/family-care/releases/tag/v0.5.0)를
게시했다. 순차 pull·up으로 `7d1a53d`/0083 전환과 세 이미지·health·API readiness 일치를
확인했다. 별도 restart 없이 WSL curl의 인증된 HTTPS·기존 저장 결과/근거/청구 조회도
30.765초에 통과했다. 실제 12개 금액/통화 보강과
