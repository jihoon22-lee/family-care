...
6. in_progress — B07 최종 writer barrier·전환·재시작/복구와 v0.5 source/schema/image 조합을 확인한다. #87/#89는 각각 필수 CI 후 `7642a1d`/`538d9fb`로 병합했다. v0.5.0은 단위 테스트의 DB 상속, v0.5.1은 복원 시험의 서비스 컨테이너 ID 누락 때문에 게시 전에 실패했다. 두 태그를 보존하고 v0.5.2에서 환경 바인딩과 기존 browser/Worker runtime 검증의 게시 전 실행을 맞춘다. 게시·실제 배포는 실행 뒤 기록한다.
