...
v0.5.x 릴리스·태그를 제거하고 구현 기능을 유지한 채 개발 버전을 v0.5.0으로 통일한다.
마일스톤의 남은 약관 연결·금액 지원·사용 검증을 마친 뒤 v0.5.0으로 최종 릴리스·배포한다.
2026-09-10 PR #95–#97의 원문 계산·구조/실패 보존 수용 후 WP 10개 중 7개를 완료했다.
#63의 실제 가입·약관 신원/적용 연결, #69의 최종 연결 적용과 #70의 지원/사용 수용은 남아 있다.
별도 0073 검증 DB의 원문 재처리·기존 행 보존과 AI-off 조회는 통과했지만, 새 약관 identity는
확보하지 못했다. 완료한 추출/보존과 미완료한 연결을 같은 성공으로 처리하지 않는다.
이전 실행 환경과 검증 이력은 [수용 원장](../release/v0.5.0-verification.md)에서 현재 계획과 구분한다.
...
3. 각 커밋에서는 문법·계획 범위만 확인한다. PR의 계획 작업이 모두 끝나면 root agent가 전체 diff와 필요한 상세 검증을 한 번 집중 검토하며, 같은 CI 검사를 로컬에서 중복하지 않는다.
4. 기능·동작 변경은 수용 조건과 실패 경계를 검증하는 테스트를 함께 작성하고 PR 계획 완료 시 실행한다.
