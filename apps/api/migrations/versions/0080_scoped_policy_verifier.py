"""Admit explicit field-scoped verification while preserving earlier review outcomes."""

from collections.abc import Sequence

from alembic import op

revision: str = "0080_scoped_policy_verifier"
down_revision: str | Sequence[str] | None = "0079_cited_rider_name"
...
def _admission(expanded: bool) -> None:
    ...
    current = previous[:-1] + ",'retained-policy-association-v11')"
    if not expanded:
        ...
        op.execute(f"""
          DO $$ DECLARE definition TEXT; BEGIN
            ...
            IF strpos(definition,$previous${previous}$previous$)=0 THEN
              RAISE EXCEPTION 'scoped policy verifier contract changed';
            END IF;
    ...
    op.execute("""
      DO $$ BEGIN
        IF EXISTS(SELECT 1 FROM policy_structuring_jobs
            WHERE pipeline_version='retained-policy-association-v11') THEN
          RAISE EXCEPTION 'scoped policy verifier history prevents downgrade' USING ERRCODE='23514';
        END IF;
      END $$;
    """)
    _admission(False)
