"""Append independently proven header regions without changing prior metadata history."""

from collections.abc import Sequence

from alembic import op

revision: str = "0073_metadata_header_regions"
down_revision: str | Sequence[str] | None = "0072_metadata_physical_flow"
    ...
    op.execute("""
        DO $$ BEGIN
          IF EXISTS(SELECT 1 FROM document_metadata_proposals
            WHERE revision='document-metadata-v11')
            OR EXISTS(SELECT 1 FROM document_metadata_publications
              WHERE validator_revision='document-metadata-api-v11') THEN
            RAISE EXCEPTION 'metadata header history prevents downgrade' USING ERRCODE='23514';
          END IF;
        END $$;
        ALTER TABLE document_metadata_publications
          DROP CONSTRAINT document_metadata_publications_validator_revision_check,
