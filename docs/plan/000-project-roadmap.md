...
- 상태: v0.5.2 컨테이너·GitHub Release 및 승인된 WSL 배포/재시작 검증 완료;
  보호 자료 연결·AI/기기 수용 잔여로 현재 마일스톤 OPEN
- 현재 구현 기준 설계: `docs/design/v0.1-product.md`; v0.5 전환 기준은 아래 현재 마일스톤 참조
- 현재 코드 버전: `v0.5.3`; 태그·이미지·실제 적용 상태는 [v0.5 수용 원장](../release/v0.5.0-verification.md)에서 별도 추적
...
PR #91/#92와 고정 20건 실제 선택 검수 평가는 통합했으며, v0.5.3 게시 전 버전 정렬을 검증 중이다. v0.5.2 게시·WSL 전환은 확인했으며 자동 노트 생성 실패의 수동 복구를 구분한다. GitHub의 실제 이슈·PR·실행 증거와 수용 원장을 확인한다. 이 문서는 완료율을 복제하지 않는다.
