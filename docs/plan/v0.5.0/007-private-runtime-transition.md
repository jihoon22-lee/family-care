...
1. complete — API/Worker는 설치된 지원 schema와 필수 계약을 검사한다. 후속 보관 증권 재처리 코드는 0065를 요구하며 기존 보호 환경의 0064 수용과 구분한다. 비밀/개인 자료 runner 생성 전·다음 작업 전과 API 업무 경로를 막으며 AI-off는 정상 준비 상태를 유지한다.
