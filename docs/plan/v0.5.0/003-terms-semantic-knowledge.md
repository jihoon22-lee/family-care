...
현재 증거: PR #78 기반 `805ad9c` CI 7/7, 이후 로컬 원문 영역 계획·0053의 재개 가능한 처리와
현재 root 페이지/legacy adapter/Worker 입력 최소화 경계를 추가했다. 같은 합성 원문 65개
Article의 전량 저장·조회와 관련 PostgreSQL 26개를 확인했다. Worker queue→API publication,
새 원문 변경 영향과 현재 안내 소비자 인계 및 최종 전체 검증은 이어서 수행한다.
