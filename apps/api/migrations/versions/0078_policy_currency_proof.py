"""Admit explicit retained currency proofs without rewriting earlier draft receipts."""

from collections.abc import Sequence

from alembic import op

revision: str = "0078_policy_currency_proof"
down_revision: str | Sequence[str] | None = "0077_source_scoped_identity"
...
def _admission(expanded: bool) -> None:
    ...
    current = previous[:-1] + ",'retained-policy-association-v9')"
    if not expanded:
        ...
        op.execute(f"""
          DO $$ DECLARE definition TEXT; BEGIN
            ...
            IF strpos(definition,$previous${previous}$previous$)=0 THEN
              RAISE EXCEPTION 'policy currency proof contract changed';
            END IF;
    ...
    if expanded:
        revisions += ",'policy-draft-normalization-v4'"
    op.execute(f"""
    ...
    op.execute("""
      DO $$ BEGIN
        IF EXISTS(SELECT 1 FROM policy_structuring_jobs
            WHERE pipeline_version='retained-policy-association-v9')
          OR EXISTS(SELECT 1 FROM policy_range_replay_sources
            WHERE normalization_revision='policy-draft-normalization-v4') THEN
          RAISE EXCEPTION 'policy currency proof history prevents downgrade' USING ERRCODE='23514';
        END IF;
      END $$;
    """)
    _admission(False)
