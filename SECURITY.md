...
개발·보안 수정 기준은 `main`이며 공개 실행 기준은 최신 GitHub Release입니다. 현재 코드의
개발 대상 버전은 `0.5.0`이며 최종 릴리스는 아직 게시하지 않았습니다. 프로젝트는 아직 `1.0.0` 이전이므로 이전 릴리스에 대한 별도
backport를 약속하지 않습니다. 보안 수정은 우선 `main`에
