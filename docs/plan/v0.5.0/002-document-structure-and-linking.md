...
후속 Task 3/4 (`fix/explicit-enrollment-amounts`, in_progress)는 명시적 단위를 적용하지 않은
기존 초안 때문에 비어 있는 가입금액·통화를 복구한다. 같은 이미 인용된 native 가입 행의
이름·숫자·단위가 유일하게 입증되는 경우만 retained v12 / normalization v6 초안을 만들고
새 독립 검수를 거친다. 기존 프로그램 게시의 빈 금액·통화만 원장 버전·사용자 결정·원문
신원 검사 후 보충한다. 과거 raw/정규화의 정확한 거부와 게시 이력은 보존한다. 아직 실행하지
않은 범위는 처리 누락과 구분하고, 완성된 구현·테스트·문서를 PR 한 묶음으로 검증한다.

현재 Task 3/4의 다음 묶음은 `fix/proven-policy-draft-context`의 명시적 v13/v7 복구다.
이전 검수가 인용한 표 헤더가 실제 전송·프로그램 증명을 통과했으나 초안 필드에 아직
연결되지 않아 거부되는 경계를 수정한다. 새 검수 전에 같은 필드값의 증명된 문맥을
