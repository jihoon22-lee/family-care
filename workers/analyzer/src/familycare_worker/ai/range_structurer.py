    ...
    *,
    ...
    response = provider.complete(
        ...
        system_instruction=(
            ...
            "enrollment. Every field must itself cite primary policy Evidence supporting "
            "that field; context-only support is insufficient. A policy_contract requires "
            "insurer and product_name grounded in the source wording. Dates are optional: "
            "omit unsupported optional fields instead of guessing a value or citation. "
            "For a date, the cited text must contain exactly one field-appropriate label "
            "followed by its date. Supported labels are contract_start: 계약시작일, 계약개시일, "
            "보험시작일, 보험개시일, contract start; contract_end: 계약종료일, 계약만기일, "
            "보험종료일, 보험만기일, contract end; coverage_start: 보장시작일, 보장개시일, "
            "coverage start; coverage_end: 보장종료일, 보장만기일, coverage end. "
            "contract_start denotes commencement, not the separate actual contract date. "
            "Do not derive these date fields from 계약일, generic 보험기간, unlabeled dates, "
            "payment periods, or maturity ages. Use NO_ENROLLMENT_FACTS for clearly "
            "non-enrollment content; use UNRESOLVED when enrollment facts may exist but "
            "cannot support a retained field. Never attach an unrelated candidate. "
            "Do not infer current active status or payment amounts from sum assured. "
