...
4. in_progress (고정 전체 평가) — 앞선 한 개발 사례의 실제 모델 진단 4회와 보관 증권의 독립 검수 1회는 제한된 표본으로 보존한다. 기존 dev/holdout 20개 사례와 기대값을 유지한 합성 원문/DB adapter로 정상 로컬 약관 해석과 실제 SDK MockTransport→Worker→API 원문 검증을 대조했다(22개 통과). 확정 진단 입력 누락·치료 종류 보존·동일 의미의 검수 불일치를 수정하며 0070 compiler v2의 이력 보존을 검증한다. 중복 호출 없는 제한형 실제 평가와 새 패치 검증은 진행 중이다. 사례를 분모에서 빼거나 기대 정답을 원문·provider 입력에 넣지 않는다. 실제 호출·개선/새 오류·토큰·통화 비용은 실행 뒤 기록한다.
