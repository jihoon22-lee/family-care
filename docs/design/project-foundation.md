...
Node.js와 Python은 승인된 지원 계열을 저장소 설정과 CI에서 일치시킨다. 애플리케이션 라이브러리는 범위를 선언하고 잠금 파일에 정확한 해석 결과를 기록한다. 컨테이너 베이스 이미지는 Dockerfile의 완전한 버전 태그를 기준으로 삼고, 정적 검사는 패치 값을 별도로 복제하지 않은 채 승인 계열·단계 순서·태그 형식을 검증한다. nginx runtime처럼 별도 승인이 필요한 이미지는 정확한 태그까지 검사한다.
