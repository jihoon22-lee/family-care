"""A revised layout proof appends metadata without discarding previous history."""
...
def test_v8_reprocessing_preserves_v7_and_refuses_destructive_downgrade(
    ...
    with psycopg.connect(_psycopg_url(url), row_factory=dict_row) as connection:
        ...
        previous.update(revision="document-metadata-v7", components=[], unresolved_pages=[1])
        old = connection.execute(
            "INSERT INTO document_metadata_proposals "
            "(generation_id,revision,state,attempts,proposal_json) "
            "VALUES(%s,'document-metadata-v7','PREPARED',1,%s) "
            "RETURNING id,to_jsonb(document_metadata_proposals)::text AS snapshot",
            (generation, Jsonb(previous)),
        ).fetchone()
    assert DocumentMetadataRunner(url).run_once("synthetic-prefix-worker")
    assert DocumentMetadataProjector(url).project_pending() == 1
    assert not DocumentMetadataRunner(url).run_once("synthetic-prefix-worker")
    with psycopg.connect(_psycopg_url(url), row_factory=dict_row) as connection:
        ...
        ).fetchall() == [{"validator_revision": "document-metadata-api-v8", "outcome": "APPLIED"}]
        snapshots = connection.execute(
            "SELECT id,to_jsonb(p)::text AS snapshot FROM document_metadata_proposals p ORDER BY id"
        ).fetchall()
        assert old in snapshots
    refused = _migrate(url, "downgrade", "0063_review_claim_sources")
