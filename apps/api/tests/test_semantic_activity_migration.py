"""New activity compilation never rewrites an earlier semantic publication."""
...
def test_compiler_upgrade_retains_v1_and_prevents_downgrade_with_v2_history(
    ...
    with monkeypatch.context() as legacy:
        legacy.setattr(core, "COMPILER_REVISION", "terms-semantic-compiler-v1")
        legacy.setattr(semantic_repository, "COMPILER_REVISION", "terms-semantic-compiler-v1")
    ...
    assert _migrate(url, "downgrade", "0069_policy_draft_replay").returncode == 0
    assert _migrate(url, "upgrade", "head").returncode == 0
    with monkeypatch.context() as activity:
        activity.setattr(core, "COMPILER_REVISION", "terms-semantic-compiler-v2")
        activity.setattr(semantic_repository, "COMPILER_REVISION", "terms-semantic-compiler-v2")
    ...
    with psycopg.connect(_psycopg_url(url), row_factory=dict_row) as connection:
        ...
        assert (
            ...
            == "terms-semantic-compiler-v2"
        )
    assert _migrate(url, "downgrade", "0070_semantic_activity").returncode == 0
    refused = _migrate(url, "downgrade", "0069_policy_draft_replay")
    assert refused.returncode != 0
    assert "psycopg.errors.CheckViolation" in refused.stderr
    assert "semantic activity history prevents downgrade" in refused.stderr
    with psycopg.connect(_psycopg_url(url)) as connection:
        assert connection.execute("SELECT version_num FROM alembic_version").fetchone() == (
            "0070_semantic_activity",
        )
    assert _migrate(url, "upgrade", "head").returncode == 0
