...
`document-metadata-v7`는 명시적 약관 이용 가이드 제목 아래에서 예시 표시의 용도를
설명하는 줄을 지속 예시 문맥과 구별한다. 제목·설명은 같은 페이지/layer의 원문 순서와
좌표로 연결하며, native 단어의 전체 원문·정수 주소·중복 주장도 검증한다. 같은 BLOCK은
줄 순서를 사용하고 해당 설명 줄만 제외한다. 실제 가정/지급 사례·별도 예시 도입·다른
문서 제목·앞 문서의 제한은 유지한다. 원문이 불완전하거나 가이드 제목이 뒤에 있으면
기존 제한을 유지한다. v1~v6 해석을 보존하며 0051은 v7 이력의 손실을 막는다.

`document-metadata-v10`은 원본 단어의 추출 ordinal을 보존하면서 검증된 물리 흐름을
별도로 읽는다. 같은 layer·열·인접 영역의 본문과 제목/metadata 상단 구간을 구분하며,
겹침·미확인 좌표·원단어 연결 오류는 증거로 건너뛰지 않는다. 역순 연결에서는 같은 layer의
좌표 없는 조각이 어느 ordinal에 있든 흐름을 보류한다. API의 독립 분류, 조항 대조와
semantic source reader도 저장된 metadata revision을 사용한다. v1–v9 이력은 유지한다.

피보험자 조건 뒤의 명시적 지급/부지급 문장도 내용 역할의 근거로 인식한다. 이는 가입이나
지급 조건 충족을 확정하지 않는다. 상품 표제는 제한된 수의 균형 잡힌 괄호와 종/형 구분을
허용하되 원래 문자열을 보존한다. 예시·설명 문맥을 정상 본문 또는 반복 상품 표제만으로
종료하지 않는다. 0072는 새 validator를 허용하고 v10 제안/게시 이력의 손실을 막는다.


`document-metadata-v11`은 기존 상단 흐름이 분리된 열 때문에 막힌 경우 전체 페이지에서
최초 물리 행부터 명시적 문서 제목과 인접 metadata 구간을 별도로 증명한다. 제목 앞에는
보험사/상품 표제 문법 또는 유효한 명시 필드만 같은 인접 흐름으로 허용한다.
제목에 도달하지 못한 표제는 역할 근거가 아니며 짧은 회사명 suffix만으로 시작하지 않는다. 줄을 대표하는
원단어의 완전한 계보·중복 소유권, 실제 사각형 교차, 같은 layer/열·거리와 중간 장애물을
API와 Worker가 독립 검증한다. 다른 열·하단·앞 본문 이후의 제목은 새 시작점이 아니다.
선택하지 않은 필드는 unresolved로 보존하고 전체 페이지·앞 페이지의 참고/예시 문맥은
그대로 검사한다. 미인식 앞 문장이나 위치가 불명확한 원문은 이 fallback의 지원 범위가 아니다.

추가 증명은 페이지 10,000 nodes/1 MiB 문자, 비교 65,536회와 반복 텍스트 검사 4 MiB로
제한한다. 표의 중복 표현 탐색과 metadata 연결의 전체 순회를 같은 예산에 포함하며,
초과한 prefix 일부를 완성된 증거로 반환하지 않는다. 제목만으로 신원·판본·가입을 만들지
않으며, 0073은 v1–v10 게시를 보존하고 v11 제안/게시가 있으면 downgrade를 거부한다.

v11의 정확 회사 표기 어휘는 2026-09-10에 열람한
[생명보험협회 정회원](https://www.klia.or.kr/klia/company/member/list.do)과
[손해보험협회 회사별 공시정보](https://consumer.knia.or.kr/m/main.do)의 게시명에서 가져온다.
원본 계약에 출처·관찰일·어휘 버전을 기록하고 API/Worker에 함께 생성한다. 이 목록은
회사 표기의 종류를 알아보는 자료이며 현재 인가·법인명·약칭/승계 관계의 판정표가 아니다.
전체 표기의 정확 일치만 허용하고 값은 원문대로 유지한다. 새 어휘에서 얻는 보험사 fact는
위의 전체 페이지 header 증명과 기존 직접 인접성을 모두 요구한다. 부분 문자열·그룹/대리점
이름·자동 약칭 확장으로 신원·가입·판본 적용을 만들지 않으며 과거 revision은 이 어휘를 쓰지 않는다.
